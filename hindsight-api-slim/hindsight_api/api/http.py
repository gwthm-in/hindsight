...
def _register_routes(app: FastAPI):
    ...
    async def api_list_mental_models(
        ...
        Defaults to metadata only. Content is opt-in via ``detail=content``/``full``
        rather than the default: returning every model's synthesized content by
        default bloated callers' context and let one list pull a whole bank's
        synthesized knowledge in bulk. To read one model, prefer
